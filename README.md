...
在 Phase 3 探索 FP8 PTX 指令时，遇到了底层硬件未完全文档化（Undocumented Behavior）的对齐陷阱，并进行了深度的逻辑推导与排查：

### 1. FP8 模式下 `ldmatrix` 的解耦限制
...
### 2. Tensor Core 累加器 (`d0`~`d3`) 寄存器非线性布局 (Quad-Pair Layout)
* **问题**：`m16n8k32` 计算结果数值正确，但按 Thread ID 写回 Global Memory 后，物理矩阵坐标发生错位。
* **归因推导**：Warp 内 32 个线程在执行 `mma.sync` 后，每个线程持有的 `d0`~`d3` 寄存器并不对应连续的矩阵块，而是遵循特定的 Quad-Pair 分布规则（例如 Lane 0~3 协同交错处理矩阵的前几行）。如果未经过寄存器间的 Shuffle 重排或物理坐标重映射直接写回，会导致逻辑矩阵被“切碎”。
